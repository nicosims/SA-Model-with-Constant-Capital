...
{ 
  ...
  while (am<length(a))
  {
      ...
      ##Hiring process 
    {
      ...
      #wsample<-sample(c(agent["wage_exp",a[,am]], round(agent["wage_exp",a[,am]]*1.15)), 1)
      ##selects wage offer, either wage expectation or 5% more than wage expectation
      wsample<-agent["wage_exp",a[,am]];
      ...
      if ((((agent["Money", E1]))>(wsample*(1+ (CperWR/10) ))) &(wsample>agent["wage_level",a[,am]])) {
        ...
        agent["wage_exp",a[,am]]<-round(agent["wage_exp",a[,am]]*1.05);
      ...
      { 
        ##if agent isn't hired, decrease wage expecations by 10%
        if (agent["Employ_ind", a[,am]]==0)
        {
        
        agent["wage_exp",a[,am]]<-round(agent["wage_exp",a[,am]]*.35);
        }
      }
    }
    ...
    if (agent["Employ_ind",conagent]<0)
    {
      EX1<- sample(1:round(agent["Money",conagent]*srate), 1);
      conexT<-conexT+EX1;
...
ggplot(data = rop, aes(x=RoP))+geom_histogram(aes(y=stat(density)))+stat_function(
  fun = dgamma,  args = list(.2, shape=.5), col = "orange")+xlim(0,17)
...
indf["Var1"]<-var1[2:length(var1)]
pareto3<-(rpareto(1000000, scale = 1e3, shape = 5.3)*.5)
...
ggplot(data = indf, aes(x=Var1, y=Freq))+geom_point()+xlab("Capitalist Income")+geom_line(data = pareto2, 
                                                                                          aes(x=Income, y=Freq), col ="orange")+scale_x_log10(limit = c(1000,5000))+scale_y_log10()
...
ggplot(data = dfWorkMon, aes(x=Worker_income))+geom_density()+stat_function(
  fun = dgamma,  args = list(.03, shape=2.5), col = "orange")+xlim(15,250)
mean(RevM[EmployM>=0])
...
ggplot(data = dfWorkMon, aes(x=capitalist_income))+geom_density()+stat_function(
  fun = dpareto,  args = list(scale = 1000, shape=4), col = "orange")+xlim(1000,7500)+ylim(0,.003)
mean(RevM[EmployM>=0])
